...
export const Projects: React.FC<ProjectsProps> = ({ projects, isDark }) => {
  ...
  return (
    <section id="research" className="py-12 border-t border-rule dark:border-slate-800">
            ...
            <div
              ...
              className={`rounded-lg border p-5 flex flex-col justify-between transition-all duration-150 hover:shadow-sm ${
                isDark
                  ...
                  : 'bg-surface border-rule hover:border-slate-300'
              }`}
            >
              <div>
                {/* Header with Project Index and Category */}
                <div className="flex items-center justify-between text-xs text-muted dark:text-slate-400 mb-2">
                ...
                <h3 className="text-base font-bold text-ink dark:text-white leading-snug mb-2">
                  {project.title}
                </h3>

                <p className="text-xs sm:text-sm text-ink-soft dark:text-slate-300 leading-relaxed mb-4">
                  {project.description}
                </p>

                {/* Highlights */}
                {project.highlights && project.highlights.length > 0 && (
                  <ul className="list-disc list-outside ml-4 space-y-1 text-xs text-ink-soft dark:text-slate-200 mb-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[11px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-ink-soft dark:text-slate-300"
                    >
                      {tag}
                    </span>
                  ))}
