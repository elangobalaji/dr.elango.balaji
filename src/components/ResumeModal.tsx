...
export const ResumeModal: React.FC<ResumeModalProps> = ({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10 bg-black/75 backdrop-blur-sm animate-fade-in print:p-0 print:bg-white"
      onClick={onClose}
    >
      <div
        id="resume-modal-content"
        className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white text-slate-900 p-8 sm:p-12 shadow-2xl border border-slate-200 print:max-h-none print:shadow-none print:border-none print:p-0"
      ...
      >
        ...
        <div className="space-y-6 text-slate-800">
          ...
          <div className="border-b border-slate-300 pb-5 text-center sm:text-left">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600 mt-2.5 font-medium justify-center sm:justify-start">
            ...
            <div className="space-y-4">
              {experiences.map((exp) => (
                <div key={exp.id}>
                  ...
                  <p className="text-xs text-slate-600 mb-1.5 leading-normal">{exp.description}</p>
            ...
            <div className="space-y-2">
              {education.map((edu) => (
                <div key={edu.id} className="text-xs">
                  ...
                  {edu.details && <p className="text-slate-600 mt-0.5">{edu.details}</p>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-xs">
              <div>
                <span className="font-semibold text-slate-900">Electrochemical Diagnostics:</span>{' '}
                <span className="text-slate-600">
                  CC/CV Testing, EIS, GITT, Voltammetry, Isothermal Calorimetry
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-900">In-situ / Operando Characterization:</span>{' '}
                <span className="text-slate-600">
                  Synchrotron-XAS, TXM, HR-XRD, Raman Spectroscopy, Confocal Optical Microscope (OM)
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-900">Electrode Synthesis & Fabrication:</span>{' '}
                <span className="text-slate-600">
                  Doctor blade coating, hydrothermal, wet chemical synthesis, slurry optimization
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-900">Cell Assembly & Custom Fixtures:</span>{' '}
                <span className="text-slate-600">
                  Operando coin cells, specialized operando optical/X-ray cells, KP cells (solid-state)
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-900">Computation & Visualization:</span>{' '}
                <span className="text-slate-600">
                  Machine learning for battery safety, Blender 3D (modelling, geometry nodes, animations), Python data analysis
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-900">Core Domains:</span>{' '}
                <span className="text-slate-600">
          ...
          {grants.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5 text-xs">
                {grants.map((grant) => (
                  <div key={grant.id}>
                    <span className="font-bold text-slate-900">{grant.title}</span> —{' '}
